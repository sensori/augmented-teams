...
2. **`use_class_based_organization.json`** - Organize tests in classes with helpers
3. **`use_arrange_act_assert.json`** - Given-When-Then structure in every test
4. **`test_observable_behavior.json`** - Test behavior through public API

### Test Quality Rules (Core)

5. **`business_readable_test_names.json`** - Test names read like plain English (BDD Rule 1)
6. **`consistent_vocabulary.json`** - One word per concept: create/verify/load (Clean Code 2.2)
7. **`use_descriptive_function_names.json`** - Intention-revealing names
8. **`match_specification_scenarios.json`** - Match specification scenarios exactly

### Test Implementation Rules

9. **`call_production_code_directly.json`** - Call real code, let tests fail naturally (BDD Rule 8.4)
10. **`cover_all_behavior_paths.json`** - Test normal, edge, and failure paths (BDD Rule 3)
11. **`mock_only_boundaries.json`** - Mock external APIs only, not business logic (BDD Rule 8.2)
12. **`use_real_implementations.json`** - Use real temp files, not mocks
13. **`helper_extraction_and_reuse.json`** - Extract duplicate setup to helpers/factories (BDD Rules 8.3 & 4)
14. **`test_driven_development.json`** - RED-GREEN-REFACTOR cycle

### Production Code Rules

15. **`production_code_single_responsibility.json`** - Each function does ONE thing
16. **`production_code_explicit_dependencies.json`** - Constructor injection
17. **`production_code_small_functions.json`** - Functions under 20 lines
18. **`production_code_api_design.json`** - Auto-init, ask-don't-tell, properties over methods (BDD Rule 11)

### Utility Rules

19. **`define_fixtures_in_test_file.json`** - Fixtures in test file, not conftest
20. **`use_ascii_only.json`** - ASCII characters only (Windows compatibility)
21. **`use_exact_variable_names.json`** - Use exact variable names from specification consistently
...
1. **Business-Readable Language (BDD Rule 1)** → `business_readable_test_names.json`
2. **Consistent Vocabulary (Clean Code 2.2)** → `consistent_vocabulary.json`
3. **Comprehensive Coverage (BDD Rule 3)** → `cover_all_behavior_paths.json`
4. **Helper Extraction (BDD Rules 8.3 & 4)** → `helper_extraction_and_reuse.json`
5. **Mock Only Boundaries (BDD Rule 8.2)** → `mock_only_boundaries.json`
6. **Natural Test Failures (BDD Rule 8.4)** → `call_production_code_directly.json`
7. **Object API Design (BDD Rule 11)** → `production_code_api_design.json`
8. **ASCII Only (BDD Rule 10)** → `use_ascii_only.json`
