"""
Test sync-render with layout preservation.

Given -> When -> Then workflow:
1. Given: Story graph JSON
2. When: Render to DrawIO, sync back to JSON (extracting layout), render again with layout
3. Then: Assert JSON integrity (sync didn't lose data) and layout preservation (re-render matches original)
"""
import sys
import io
...
def main():
    ...
    print(f"\n{'='*80}", flush=True)
    print("SYNC RENDER WITH LAYOUT PRESERVATION TEST", flush=True)
