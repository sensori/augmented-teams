...
def get_original_drawio_path() -> Path:
    ...
    # Fallback to simple_story_graph actual file (for testing)
    simple_actual = acceptance_dir / "scenarios" / "simple_story_graph" / "3_then" / "actual-first-render.drawio"
    if simple_actual.exists():
        return simple_actual
    ...
    # If none exist, return the expected path (will fail with clear error)
    return original_drawio
