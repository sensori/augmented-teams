"""
Assert positioning preservation after sync-then-render workflow.

THEN: Assert expected matches actual (DrawIO layout is all we care about, JSON not important)

UNIQUE TO THIS ASSERTION:
- Asserts sync-then-render workflow (sync from DrawIO → render with extracted layout)
- Asserts single render: expected vs actual (after sync-then-render)
- Asserts that users are positioned over stories correctly
- Asserts that all elements preserve positioning after sync-then-render cycle
- Asserts positioning preservation for complex diagrams with many users and stories
"""
import sys
