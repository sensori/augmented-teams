...
- Loads input story graph JSON (if input folder exists)
...
def get_story_graph() -> Dict[str, Any]:
    ...
    Uses input folder if it exists, otherwise uses expected JSON.
...
def get_story_graph_path() -> Path:
    ...
    Checks input folder first, then falls back to expected JSON.
    
    Returns:
        Path to story graph JSON file
    """
    # Check for input folder first
    input_dir = scenario_dir / "input"
    input_file = input_dir / "input_story_graph.json"
    if input_file.exists():
        return input_file
    
    # Fall back to expected JSON
    return given_dir / "story-graph-complex.json"
