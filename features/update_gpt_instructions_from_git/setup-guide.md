...
The startup script expects `config/bootstrap-config.json`, but we moved it to `src/features/gpt-builder-config/`. Update the path:

```bash
# Copy config to expected location
cp src/features/gpt-builder-config/bootstrap-config.json config/
```

### 4.2 Test Startup Script
```bash
# Test the startup script
cd src/features/gpt-builder-config
...
```json
{
  "auto_refresh": {
    "instruction_paths": [
      "instructions/",
      "config/",
      "src/features/gpt-builder-config/"
    ]
  }
}
```
