...
Path("features/containerization/logs").mkdir(parents=True, exist_ok=True)
...
def run_deployment(feature_name: str, requirements: str, mode: str = "CONTAINER"):
    ...
    cmd = [
        "python", "features/containerization/ai-assisted-deploy.py",
...
def get_latest_log():
    ...
    logs_dir = Path("features/containerization/logs")
...
class TestErrorRecovery:
    ...
    def test_missing_dependency_detection(
        ...
        result = subprocess.run(
            ["python", "-c", f"import sys; sys.path.insert(0, 'features/{test_feature_name}'); import main"],
            capture_output=True,
            text=True
        )
