Hooks.on("ready", () => {  
    ...
    game.settings.register("mm3e-animations", "animateOnAttack", {
        name: "Animate on Attack",
        hint: "Automatically animate when attacks are made",
        scope: "world",
        config: true,
        type: Boolean,
        default: true
    ...
    game.settings.register("mm3e-animations", "showAnimationButton", {
        name: "Show Animation Button on Chat Cards",
        hint: "Display the 'Run Animation' button on attack chat cards",
        scope: "world", 
        config: true,
        type: Boolean,
        default: true
    ...
    game.settings.register("mm3e-animations", "animateOnMovement", {
        name: "Animate on Movement",
        hint: "Automatically animate when tokens move based on their movement powers",
        scope: "world",
        config: true,
        type: Boolean,
        default: true
    ...
    // Initialize movement detection system
    ...
    // Function to run attack animation manually
    async function runAttackAnimation(message, html, data) {
      try {
        // Get the actor from the speaker
        const speaker = data.message.speaker;
        const actor = game.actors.get(speaker.actor);
        if (!actor) {
          ui.notifications.error("Actor not found for animation");
          return;
        }
        
        // Get the token for the actor
        let token = null;
        if (speaker.token) {
        ...
        if (!token) {
          ui.notifications.error("Token not found for animation");
          return;
        }
        
        // Parse the attack data from the HTML content
        // Look for the button with data-datk attribute
        const attackButton = html.find('button.btnRoll[data-datk]').first();
        if (!attackButton.length) {
          ui.notifications.error("No attack data found in chat message");
          return;
        }
        
        // Extract attack data from the button's data attributes
        // jQuery automatically parses data attributes, so we get the object directly
        const attackDataParsed = attackButton.data('datk');
        
        // Get the actual attack from the actor's attack collection
        const attacks = actor.system.attaque;
        let attackData = null;
        
        // Find the attack by ID in the attack collection
        for (const [key, attack] of Object.entries(attacks)) {
          if (attack._id === attackDataParsed._id) {
            attackData = attack;
            break;
          }
        }
        ...
        if (!attackData) {
          ui.notifications.error("Attack not found in actor's attack collection");
          return;
        }
        
        // Extract target ID from the button's data-target attribute
        const targetId = attackButton.data('target');
        
        if (!targetId) {
          ui.notifications.error("No target found in attack data");
          return;
        }
        ...
        // Extract the actual roll result from the chat message
        const resultElement = html.find('h4.dice-total.result span').first();
        const defenseElement = html.find('h4.dice-total.result span').eq(1);
        const hitText = html.find('h4.dice-total.attack').text();
        
        const rollTotal = resultElement.length ? parseInt(resultElement.text()) : attackData.attaque || 10;
        const defense = defenseElement.length ? parseInt(defenseElement.text()) : 10;
        const isHit = hitText.includes('hits') || hitText.includes('hit');
        const isCrit = hitText.includes('Critical') || hitText.includes('critical');
        
        // Create the actual result for animation
        const result = {
          hit: isHit,
          crit: isCrit,
          total: rollTotal,
       ...
       // await animateAttackRollResults(result, token, target, attackData);
        
        let item = token.actor.items.get(attackData.links.pwr)
        let powerItem = new PowerItem(item);
        const hasArea = attackData?.area?.has ?? false;
        ...
        if(powerItem.animation){
            powerItem.animation?.play(token); 
        }
        
      } catch (error) {
        console.error("Error running attack animation:", error);
        ui.notifications.error("Failed to run attack animation");
      }
    }
    ...
    Hooks.on('rollAttack', async (atk, token,strategie, altKey) => {
        ...
        // Check if auto-animation is enabled
        const animateOnAttack = game.settings.get("mm3e-animations", "animateOnAttack");
        if (animateOnAttack && powerItem.animation){
            powerItem.animation?.play(token); 
        }
    }) 

    Hooks.on('damageSaved', async (damageSave)=>{
        const parsedDamageSave = await parseDamageSaveFromMessage(damageSave);
        await animateDamageSave(parsedDamageSave)
    
    })
    

    async function parseDamageSaveFromMessage(damageSave) {
        // Extract basic info from the message
        const speaker = damageSave.speaker;
        const roll = damageSave.rolls[0];
        const rollTotal = roll.total;
        
        // Parse HTML content to extract DC and success/failure
        const content = damageSave.content;
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = content;
        
        // Get target token ID from speaker data
        let tokenId = speaker.token;
        if(!tokenId){
            let actorId = speaker.actor;
            if(actorId){
                let token = canvas.tokens.placeables.find(token => token.actor.id === actorId);
                if(token){
                    tokenId = token.id;
                }
            }
                ...
                if(alias){
                    let token = canvas.tokens.placeables.find(token => token.actor.name === alias);
                    if(token){
                        tokenId = token.id;
                    }
                }
        ...
        // Extract DC from the HTML
        const resultElement = tempDiv.querySelector('h4.dice-total.result');
        const dcMatch = resultElement?.textContent?.match(/vs.*?(\d+)/);
        const dc = dcMatch ? parseInt(dcMatch[1]) : 0;
        
        // Determine success/failure
        const attackElement = tempDiv.querySelector('h4.dice-total.attack');
        const isSuccess = attackElement?.textContent?.includes('succeeds') || false;
        
        // Determine defense type from the flavor text and content
        const flavorElement = tempDiv.querySelector('h4.dice-total.flavor');
        const flavorText = flavorElement?.textContent?.toLowerCase() || '';
        let defenseType = "dmg"; // Default to damage
        
        // Check if it's an affliction save based on flavor text and content
        // Afflictions can be resisted by Will, Fortitude, or Toughness
        // Look for keywords that suggest affliction effects
        const contentText = content.toLowerCase();
         // French system IDs mapped to English values for detection
         const conditionKeywords = [
             'controlled', 'impaired', 'fatigued', 'disabled', 'dazed', 
             'immobile', 'unaware', 'debilitated', 'hindered', 'defenseless',
             'transformed', 'vulnerable', 'staggered', 'entranced', 'compelled',
             'exhausted', 'bound', 'dying', 'incapacitated', 'surprised',
             'weakened', 'prone', 'blind', 'asleep', 'restrained', 
             'paralyzed', 'deaf', 'stunned'
         ];
        if (contentText.includes('damage') || 
                     contentText.includes('injury') || 
                     contentText.includes('hurt')) {
                defenseType = "dmg";
            }
            else if(conditionKeywords.some(keyword => contentText.includes(keyword))) {
                defenseType = "affliction";
            }
            else {
                defenseType = "weaken";
            }
        ...
        let degreesOfFailure = 0;
        if (!isSuccess) {
            degreesOfFailure = Math.max(1, Math.ceil((dc - rollTotal) / 5));
        }
        
        // Define condition keywords once (using valid conditions from better-attacks)
       
        
        // Check if the content suggests affliction effects using the same keywords
        if (conditionKeywords.some(keyword => contentText.includes(keyword))) {
            defenseType = "affliction";
        }
        
        // Extract actual condition from content for afflictions
        let actualCondition = null;
        if (defenseType === "affliction") {
            for (const keyword of conditionKeywords) {
                if (contentText.includes(keyword)) {
                    actualCondition = keyword;
                    break;
                }
            }
        }
        ...
        return {
            target: tokenId,
            isSuccess: isSuccess,
            degreesOfFailure: degreesOfFailure,
            ...
            failEffects: {
                ...
                affliction: [
                    { status: [actualCondition || "dazed"] },
                    { status: [actualCondition || "stunned"] },
                    { status: [actualCondition || "incapacitated"] },
                    { status: [actualCondition || "dying"] }
                ],
                weaken: [
                    { status: [actualCondition || "weaken"] },
                    { status: [actualCondition || "weaken"] },
                    { status: [actualCondition || "weaken"] },
                    { status: [actualCondition || "weaken"] }
                ]
    ...
    async function animateDamageSave(damageSave) {     
        ...
        if(damageSave.isSuccess){  
            ...
            if(damageSave.defenseType =="affliction"){
                effects = damageSave.failEffects.affliction[damageSave.degreesOfFailure-1].status
            
                let effectList = getStatusEffects();
                await GameHelper.sleep(3000)
                for(effect in effects){
                    let effectLabel = effectList.find(item => item.label.toLowerCase() === effects[effect].toLowerCase());
                    if(  !isAlly )
                        await animateTextBesideTarget(target, effectLabel.label, "green")
                    else
                        await animateTextBesideTarget(target, effectLabel.label, "red")
                }  
            }   
        }
    }   
    ...
    Hooks.on("renderActorSheet", (app, html, data) => {
        ...
        // Hook to add "Run Animation" button to attack chat cards
    Hooks.on('renderChatMessage', (message, html, data) => {
        // Check if button display is enabled
        const showAnimationButton = game.settings.get("mm3e-animations", "showAnimationButton");
        if (!showAnimationButton) return;
        
        // Check if this is an attack roll message
        if (data.message.content.includes('dice-roll mm3-roll')) {
            
            const speaker = data.message.speaker;
            
            // Only show button for the user who made the attack
            if (speaker.actor && (game.user.id === data.message.user || game.user.isGM || game.user.role >= CONST.USER_ROLES.OWNER)){
                
                // Find the resistance buttons container
                const btnContainer = html.find('.btn').last();
                if (btnContainer.length) {
                    ...
                    // Create the animation button container
                    const animationBtnContainer = document.createElement('div');
                    animationBtnContainer.className = 'btn';
                    
                    // Create the animation button
                    const animationButton = document.createElement('button');
                    animationButton.className = 'btnRoll';
                    animationButton.innerHTML = '<i class="fas fa-play"></i> Run Animation';
                    animationButton.type = 'button'; // Prevent form submission
                    
                    // Add click handler
                    animationButton.addEventListener('click', async (event) => {
                        event.preventDefault();
                        event.stopPropagation();
                        await runAttackAnimation(message, html, data);
                    });
                    
                    // Assemble the elements
                    animationBtnContainer.appendChild(animationButton);
                    
                    // Insert separator and button container after the last resistance button
                    btnContainer.after(separator);
                    separator.after(animationBtnContainer);
                }
            }
        }
    });
    ...
    return this;
        ...
        affectAffliction({affected}={})
        {
            ...
            .affectCommon()
                ...
                .filter("ColorMatrix",  {hue: 500, saturate: -1, brightness: .1,contrast: 1  })
                .persist()
                .playSound("modules/mm3e-animations/sounds/action/powers/Swarm2_loop.ogg")
                .scale(.8) 
                .persist( true)
            .zIndex(10)
            .pause(800)
    ...
    get animation(){ 
        ...
        if(!animation.name){
            ...
            if(descripterSequence.descriptorClass){
               ...
               /* animation.start = async (token) => {
                    //await GameHelper.sleep(2000)
                    // Use existing SequencerScript but modify for start phase
                    descripterSequence.sequencerScript.generateStart() 
                    descripterSequence.sequencerScript.run()
                }
                animation.end = async (token) => {
                    await GameHelper.sleep(2000)
                    // Use existing SequencerScript but modify for end phase
                    descripterSequence.sequencerScript.generateEnd() 
                    descripterSequence.sequencerScript.run()
                }*/
...
`; 
    ...
    /*async generateStart() {
        // Simple start script - just effect, cast, and play
        const descriptor = this.descriptorSequence.selectedDescriptor;
        const effect = this.descriptorSequence.powerEffectSequence.selectedEffectMethods[0];
        
        if (!effect) return;
        
        const startMethod = effect.original.replace(/^affect/, "cast");
        
        let script = `const selected = GameHelper.selected;
const position = { x: selected.x, y: selected.y };
new Sequence()
    .${descriptor}()
    .${startMethod}({ affected: selected, position: position })
    .play();`;
    ...
    async generateEnd() {
        // Simple end script - just effect, affect, and play
        const descriptor = this.descriptorSequence.selectedDescriptor;
        const effect = this.descriptorSequence.powerEffectSequence.selectedEffectMethods[0];
...
new Sequence()
    .${descriptor}()
    .${effect.original}({ affected: selected, position: position })
    .play();`;
...
// ============================================================================
// MOVEMENT DETECTION AND ANIMATION SYSTEM
// ============================================================================

// Track processing flags to prevent recursive calls

initializeMovementDetection
// Track active drag operations
let activeDrags = new Set();

function initializeMovementDetection() {
    const originalOnDragLeftStart = Token.prototype._onDragLeftStart;
    Token.prototype._onDragLeftStart = function(event) {
        
        //activeDrags.add(this.id);
        const tokenId = this.id;
        const token = canvas.tokens.get(tokenId);
        
        // Check if we should cancel the drag before starting it
        handleTokenMovementStart(token).then((animationResult) => {
            if (animationResult === false) {
                // Cancel the drag operation
                console.log(`Canceling drag for token ${tokenId} due to animation`);
                // The drag was already started, so we need to cancel it
        ...
        console.log(`Drag started for token ${this.id}`);
    ...
    Token.prototype._onDragLeftCancel = function(event) {
        ...
        // Wait for token to stop moving, then call end handler
        /*if (activeDrags.has(this.id)) {
            const tokenId = this.id;
            let lastX = this.x;
            let lastY = this.y;
            ...
            const checkMovement = async () => {
                ...
                if (token) {
                    ...
                    if (currentX !== lastX || currentY !== lastY) {
                        // Position changed, update and check again
                        lastX = currentX;
                        lastY = currentY;
                        setTimeout(checkMovement, 100);
                    } else {
                        // Position hasn't changed, token has stopped moving
                        activeDrags.delete(tokenId);
                        const tokenDocument = token.document;
                        if (tokenDocument && tokenDocument.actor) {
                            await handleTokenMovementEnd(token, {});
                        }
                    }
                } else {
                    // Token not found, clean up
                    activeDrags.delete(tokenId);
                }
            };
            
            // Wait a bit before starting to check movement
    ...
    // Hook into token updates to detect movement
    /*Hooks.on('updateToken', async (tokenDocument, updateData, options, userId) => {
        // Only process if it's the current user, not already processing, and it's a drag operation
        if (userId === game.userId && 
            !processingMovement.has(tokenDocument.id) &&
            activeDrags.has(tokenDocument.id)) {
            
            const token = tokenDocument.object;
          //  await handleTokenMovementStart(token);
        }
    });
}*/

async function handleTokenMovementStart(token) {
    try {
        // Check if movement animation is enabled
        if (!game.settings.get("mm3e-animations", "animateOnMovement")) {
            return;
        }
        ...
        const movementType = await detectMovementType(actor);
        if (!movementType) return;
        
        const movementPower = await findMovementPower(actor, movementType);
        if (!movementPower) return;
        
        const powerItem = new PowerItem(movementPower);
        if (powerItem.animation) {
           // if (powerItem.animation.type === "descriptor" && powerItem.animation.start) {
           //     await powerItem.animation.play(token);
          //  } else if (powerItem.animation.play) {
        await powerItem.animation.play(token);
        //if power is an animationtion cacel the dragf operation
        ...
        console.error("Error handling token movement start:", error);
    }
}

/*async function handleTokenMovementEnd(token, updateData) {
    try {
        // Check if movement animation is enabled
        if (!game.settings.get("mm3e-animations", "animateOnMovement")) {
            return;
        ...
        // Determine movement type based on active powers
        const movementType = await detectMovementType(actor);
        if (!movementType) return;
        
        // Find the power that provides this movement type
        const movementPower = await findMovementPower(actor, movementType);
        if (!movementPower) return;
        
        // Create PowerItem and end animation
        const powerItem = new PowerItem(movementPower);
        if (powerItem.animation) {
            if (powerItem.animation.type === "descriptor" && powerItem.animation.end) {
                console.log(`Ending ${movementType} animation for ${actor.name}`);
                await powerItem.animation.end(token);
            }
            // For non-descriptor animations, we don't play anything on end
        }
        
    } catch (error) {
        console.error("Error handling token movement end:", error);
    }
*/

async function detectMovementType(actor) {
    try {
        // Check the vitesse.list to find the selected movement type
        const vitesseList = actor.system.vitesse?.list || {};
        const selectedMovement = Object.values(vitesseList).find(movement => movement.selected);
        
        if (!selectedMovement) {
            return 'ground'; // Default to ground movement
        }
        
        // Map the movement types to animation types
        const movementType = selectedMovement.autotrade || selectedMovement.label;
        ...
        console.error("Error detecting movement type:", error);
        return 'ground';
    }
}

async function findMovementPower(actor, movementType) { 
    try {
        // Get all powers from the actor
        const powers = actor.items.filter(item => item.type === 'pouvoir');
        
        // Map movement types to power name patterns
        const movementPatterns = {
            'flight': ['flight', 'fly', 'flying', 'levitation', 'hover'],
            'speed': ['super speed', 'superspeed', 'speed', 'running'],
            'teleport': ['teleport', 'teleportation', 'blink', 'dimension door'],
            'swimming': ['swimming', 'aquatic', 'water breathing', 'amphibious'],
            'burrowing': ['burrowing', 'tunnel', 'earth glide', 'digging'],
            'ground': ['running', 'speed', 'athletics', 'movement'],
            'leaping': ['leaping', 'leap', 'super leap', 'jumping'],
        };
        
        const patterns = movementPatterns[movementType] || [];
        
        // Find the first power that matches any pattern
        for (const power of powers) {
            const powerName = power.name.toLowerCase().replace(/\d+$/, ''); // Strip trailing numbers
            const powerNotes = (power.system.notes || '').toLowerCase();
            const powerEffects = (power.system.effetsprincipaux || '').toLowerCase();
            
            // Check if power name, notes, or effects contain movement keywords
            for (const pattern of patterns) {
                if (powerName.includes(pattern) || 
                    powerNotes.includes(pattern) && movementType!='speed'|| 
                    powerEffects.includes(pattern) && movementType!='speed') {
                    return power;
                }
            }
        }
        
        // If no specific movement power found, return null
        return null;
        
    } catch (error) {
        console.error("Error finding movement power:", error);
        return null;
    }
}

function flagEffectForMovement(effect, movementType) {
    if (!effect.flags) effect.flags = {};
    if (!effect.flags['mm3e-animations']) effect.flags['mm3e-animations'] = {};
    effect.flags['mm3e-animations'].movementType = movementType;
}

// Exports for testing
export { GameHelper, PowerItem, SequenceRunnerEditor, AffectedByPowerSequence, 
         BaseSequence, PowerEffectSequence, AreaSequence, ProjectionSequence,
         TokenAnimation, CastSequence, DescriptorSequence, SequencerScript };